...
# the useful data is interleaved first before mapping is applied
...
class mapper():
    #ofdm symbol index, max carriers for current mode, one tps bit, fifo to read from, fifo to write to
    def __init__(self, globalsettings, ctx, queue, cl_thread_lock, in_fifo, out_fifo):
        ...
        self.fd = open('symbol_interleaver_mapper.cl', 'r')
        fstr = "".join(self.fd.readlines())
        self.program = cl.Program(ctx, fstr).build()

        #create the opencl kernel
        self.kernel_symbols0_6817 = cl.Kernel(self.program,"interleaver_and_map_symbols0_6817")

        #create the opencl kernel
        self.kernel_symbols1_6817 = cl.Kernel(self.program,"interleaver_and_map_symbols1_6817")

        #create the opencl kernel
        self.kernel_symbols2_6817 = cl.Kernel(self.program,"interleaver_and_map_symbols2_6817")

        #create the opencl kernel
        self.kernel_symbols3_6817 = cl.Kernel(self.program,"interleaver_and_map_symbols3_6817")

        #create the opencl kernel
        self.kernel_symbols0_1704 = cl.Kernel(self.program,"interleaver_and_map_symbols0_1704")

        #create the opencl kernel
        self.kernel_symbols1_1704 = cl.Kernel(self.program,"interleaver_and_map_symbols1_1704")

        #create the opencl kernel
        self.kernel_symbols2_1704 = cl.Kernel(self.program,"interleaver_and_map_symbols2_1704")

        #create the opencl kernel
        self.kernel_symbols3_1704 = cl.Kernel(self.program,"interleaver_and_map_symbols3_1704")

        self.fd.close()

        # opencl buffer holding the computed data
        self.dest_buf = cl.Buffer(self.ctx, cl.mem_flags.READ_WRITE, self.globalsettings.odfmmode * 8)

        # opencl buffer holding the input data
        self.src_buf = cl.Buffer(self.ctx, cl.mem_flags.READ_WRITE, self.globalsettings.odfmuseablecarriers * 8)
    ...
    def map_symbols(self):
        ...
        cl.enqueue_copy(self.queue, self.dest_buf, skeleton)
        ...
        if self.globalsettings.odfmcarriers == 6817:
            if self.symbol_counter % 4 == 0:
                self.kernel_symbols0_6817.set_args( self.src_buf, self.dest_buf)
                cl.enqueue_nd_range_kernel(self.queue,self.kernel_symbols0_6817,(self.globalsettings.odfmcarriers,),None )
            if self.symbol_counter % 4 == 1:
                self.kernel_symbols1_6817.set_args( self.src_buf, self.dest_buf)
                cl.enqueue_nd_range_kernel(self.queue,self.kernel_symbols1_6817,(self.globalsettings.odfmcarriers,),None )
            if self.symbol_counter % 4 == 2:
                self.kernel_symbols2_6817.set_args( self.src_buf, self.dest_buf)
                cl.enqueue_nd_range_kernel(self.queue,self.kernel_symbols2_6817,(self.globalsettings.odfmcarriers,),None )
            if self.symbol_counter % 4 == 3:
                self.kernel_symbols3_6817.set_args( self.src_buf, self.dest_buf)
                cl.enqueue_nd_range_kernel(self.queue,self.kernel_symbols3_6817,(self.globalsettings.odfmcarriers,),None )

        elif self.globalsettings.odfmcarriers == 1704:
            if self.symbol_counter % 4 == 0:
                self.kernel_symbols0_1704.set_args( self.src_buf, self.dest_buf)
                cl.enqueue_nd_range_kernel(self.queue,self.kernel_symbols0_1704,(self.globalsettings.odfmcarriers,),None )
            if self.symbol_counter % 4 == 1:
                self.kernel_symbols1_1704.set_args( self.src_buf, self.dest_buf)
                cl.enqueue_nd_range_kernel(self.queue,self.kernel_symbols1_1704,(self.globalsettings.odfmcarriers,),None )
            if self.symbol_counter % 4 == 2:
                self.kernel_symbols2_1704.set_args( self.src_buf, self.dest_buf)
                cl.enqueue_nd_range_kernel(self.queue,self.kernel_symbols2_1704,(self.globalsettings.odfmcarriers,),None )
            if self.symbol_counter % 4 == 3:
                self.kernel_symbols3_1704.set_args( self.src_buf, self.dest_buf)
                cl.enqueue_nd_range_kernel(self.queue,self.kernel_symbols3_1704,(self.globalsettings.odfmcarriers,),None )
        ...
        for i in range(0,self.globalsettings.odfmcarriers):
            ...
            if self.tpssarray.count(i) > 0:
                ...
                if symbol_index == 0:
                    ...
                    self.tmp = complex(self.tmp.real * tps_bit, 0)
                    self.mappedsymbols.append(self.tmp)
...
class tps():
    def __init__(self,globalsettings,frame_index):
        ...
        if frame_index > 3:
            print "tps: frame_index to big! exiting..."
            return
        ...
        if globalsettings.modulation == 2:
            ...
            print "tps: unsupported modulation: %d ! exiting..." % globalsettings.modulation
            return
        ...
        if globalsettings.coderate == 0.5:
            ...
            print "tps: unsupported coderate: %d ! exiting..." % globalsettings.coderate
            return
        ...
        if globalsettings.guardinterval == 0.25:
            ...
            print "tps: unsupported guardinterval: %d ! exiting..." % globalsettings.guardinterval
            return
        ...
        if globalsettings.odfmmode == 8192:
            ...
            print "tps: unsupported odfmmode mode: %d ! exiting..." % globalsettings.odfmmode
            return

        if (frame_index & 1) == 1:
            self.bitarray.append((globalsettings.cellid & 0x00008000)>>15)
            self.bitarray.append((globalsettings.cellid & 0x00004000)>>14)
            self.bitarray.append((globalsettings.cellid & 0x00002000)>>13)
            self.bitarray.append((globalsettings.cellid & 0x00001000)>>12)
            self.bitarray.append((globalsettings.cellid & 0x00000800)>>11)
            self.bitarray.append((globalsettings.cellid & 0x00000400)>>10)
            self.bitarray.append((globalsettings.cellid & 0x00000200)>>9)
            self.bitarray.append((globalsettings.cellid & 0x00000100)>>8)
        else:
            self.bitarray.append((globalsettings.cellid & 0x00000080)>>7)
            self.bitarray.append((globalsettings.cellid & 0x00000040)>>6)
            self.bitarray.append((globalsettings.cellid & 0x00000020)>>5)
            self.bitarray.append((globalsettings.cellid & 0x00000010)>>4)
            self.bitarray.append((globalsettings.cellid & 0x00000008)>>3)
            self.bitarray.append((globalsettings.cellid & 0x00000004)>>2)
