...
class file_creator():
    ...
    def test_algorithmA(self, ofdm_mode, guardinterval):
        ...
        for line in self.fd_input:
            ...
            #tmp = [float(0) + 1j*float(0)] * 2048
            #for i in range(0,size/2):
                #tmp[i] = data_to_encode[852+i]
                #tmp[1196+i] = data_to_encode[i]
            #data_to_encode = tmp

            #encoded_data = self.python_ifft_guardinterval_func(size, 0.25, data_to_encode)#8192
            #encoded_data = numpy.fft.ifft(data_to_encode)
            encoded_data = numpy.fft.ifft(numpy.fft.fftshift(data_to_encode))#,n=2048
            tmp = []
